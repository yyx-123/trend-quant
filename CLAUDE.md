...
- `core/trend_phase.py` — multi-period trend phase: three-state discretization (daily ±5, weekly/monthly ±9) + run length in bars, assembled as the dashboard `trend_periods` bundle. Pure computation; data access + per-level aggregation live in `services/dashboard_common.build_period_phase_index`.
- `core/rolling_bars.py` — rolling-anchored weekly/monthly trend: bar k = the k-th trailing D-trading-day window ending today (weekly D=5, monthly D=22; whole series re-anchored daily, no in-period volume/TR ramp). Reuses the canonical trend formula with period-scaled windows only (weekly ATR8/ER4/vol8, monthly ATR6/ER3/vol6). Vectorized over (days × bars); equivalence with `calculate_trend_score_series` pinned by `tests/unit/test_rolling_bars.py`. State thresholds for research/signals: daily ±5, rolling weekly/monthly ±9 (since 2026-09-14, per the 2026-09-13 rolling-trend-calibration study, archived in `research/README.md`).
...
- `data/storage/db.py` — all tables incl. `market_data_qfq/raw` (+ `_weekly`/`_monthly` vendor period bars, last bar may be the in-progress current period — see `core/bars.is_period_bar_provisional`), `market_data_qfq_*_fitted` (**dormant since 2026-09-16**: per-day in-progress bar snapshots, zero production consumers; daily-write hook removed, table/data/`refresh_fitted_period_bars` kept for manual backfill), `trend_rolling_daily` (per-day rolling-anchored weekly/monthly trend values from qfq daily bars; maintained by the daily-update hook — full rebuild on factor change, trailing rows upserted on raw append), `instrument_metadata` (sole instrument store), `rule_strategies`, `job_runs`, `app_config`, `indicator_daily`, `trend_daily`, `trend_param_sets`. WAL mode.
