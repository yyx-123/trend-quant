...
> - 修复清单与验收记录：`round2-fixes.md`；
> - 验收代理：V3 **FAIL**（2 项 P1）→ 修复 → V4 **FAIL**（驳倒"不可区分"论断 + 2 个红灯用例）
>   → 改为**精确恒等式**设计 → V5 **PASS**（无阻断项，7 项 P3）→ 逐项修复 → V6 复核
>   **2 项未闭合**（fixture 的 nav 非引擎形态使恒等式空转、饱和悬崖按种子敏感、新钉子顺序依赖）
>   → 全部修掉并**变异反证**（含 V6 判定的那条 BLOCKING：删掉数量界后伪造数量仍被恒等式抓住）；
> - 全量回归：1594 passed / 1 failed（既有 Windows flake）；ruff 与基线逐条对比新增 0 条。
>
> 日期：2026-09-25
