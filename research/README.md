...
每个子目录是一项独立的小研究，命名 `YYYY-MM-DD_主题`，自包含：
...
- `2026-09-12_趋势值分布` —— trend_score 全市场分布：股票/ETF × 日/周/月
  六组，为「正/无/负趋势」阈值划分提供数据依据。
- `2026-09-13_趋势相位迁移` —— 日/周/月三态组合的 27 种相位：首次出现后
  1周/2周/1月/2月收益，以及 27×27 迁移矩阵的频次与迁移后收益分布。
- `2026-09-13_趋势相位迁移_在途合成` —— 上一研究的口径修正：周/月趋势
