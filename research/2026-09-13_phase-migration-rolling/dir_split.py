"""按「进入方向」拆分 27 组合的收益（滚动锚定版 v3；不重算，读本目录 events.csv）。
...
口径与 v2 的 dir_split.py 逐项相同，仅数据源换成本目录的滚动口径 events.csv。

输出：
- data/combo_dir_stats.csv  to_combo × direction × 4 horizons 全套统计
- combo_dir_split.png       27 组合 × [升级/降级 × 1月/2月] 收益热力图 + 方向频次

运行：scripts/temp/plot-venv/bin/python research/2026-09-13_phase-migration-rolling/dir_split.py
"""
...
def plot(events: pd.DataFrame) -> None:
    ...
    fig.suptitle("趋势相位组合：按进入方向（升级/降级/换挡）拆分 · 滚动锚定口径", fontsize=12)
