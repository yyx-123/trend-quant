"""按「进入方向」拆分 27 组合的收益（新阈值版 v4；不重算，读本目录 events.csv）。
...
口径与 v3 的 dir_split.py 逐项相同，仅数据源换成本目录的 v4（日±5/周月±9）
events.csv。
...
运行：scripts/temp/plot-venv/bin/python research/2026-09-14_phase-migration-th9/dir_split.py
"""
...
def plot(events: pd.DataFrame) -> None:
    ...
    fig.suptitle("趋势相位组合：按进入方向（升级/降级/换挡）拆分 · v4 新阈值（日±5/周月±9）", fontsize=12)
...
def main() -> None:
    ...
    tot = events.groupby("direction", observed=True).size()
    print(f"\n方向总量：升级 {int(tot.get('升级', 0)):,} / 降级 {int(tot.get('降级', 0)):,} "
          f"/ 换挡 {int(tot.get('换挡', 0)):,}")
