...
>
> **口径变更注记（2026-09-14）**：§6.1 的 ±9 建议已被采纳并落地——三态阈值自
> 2026-09-14 起为 **日 ±5（不变）、滚动周/月统一 ±9**（±9 实测见 §4.1 末表）。
> 依赖该阈值的后续研究：`research/2026-09-14_phase-migration-th9/`
> （相位迁移 v4，日±5/周月±9）。
...
**±9 口径实测**（2026-09-14 起采纳的周/月三态阈值，见 §6.1；已补入
`data/stats.json` 四个滚动组的 `pct_*_9` / `pctile_rank_at_*9` 字段）：

| 组 | 按 ±9：负/无/正 | ±9 分位 q(−9)/q(+9) |
|---|---|---|
| 股票·滚动周 | 13.2% / **69.0%** / 17.8% | 13.2% / 82.2% |
| 股票·滚动月 | 12.5% / **67.0%** / 20.4% | 12.5% / 79.6% |
| ETF·滚动周 | 14.7% / **66.3%** / 19.1% | 14.7% / 80.9% |
| ETF·滚动月 | 14.8% / **63.1%** / 22.1% | 14.8% / 77.9% |

±9 下四组「无趋势」占比恢复到 63%~69%，回到日K ±5 口径的 ~2/3 水平；
±9 所处分位 ≈ p12.5~p14.8 / p77.9~p82.2，与日K ±5 卡的 p15/p82 基本对齐，
且保持右偏结构（正趋势多于负趋势）。
...
1. **三态阈值（已采纳，2026-09-14 起）：日 ±5 不变，滚动周/月统一 ±9。**
   日K 维持 ±5（本研究复现了前序结论：~2/3 无趋势、p15/p82）。滚动周/月
   放宽到 ±9 的原推理（历史记录）：要求「无趋势为多数」与日K 语义对齐时
   ±5 偏窄，实测恢复 2/3 无趋势所需的对称阈值为股票周 8.3 / 股票月 8.9 /
   ETF 周 9.1 / ETF 月 10.1，取整统一 ±9；按日K 相同分位（p15.3/p82.1）
   折算为股票周 [-7.9, +9.0]、股票月 [-7.6, +10.8]，同量级、略右偏。
   **落地实测**（§4.1 末表，`data/stats.json` 的 `pct_*_9` 字段）：±9 下
   「无趋势」占比恢复到 股票周 69.0% / 股票月 67.0% / ETF 周 66.3% /
   ETF 月 63.1%（≈ 日K ±5 的 2/3 水平），±9 卡在 p12.5~p14.8 /
   p77.9~p82.2，与日K ±5 的分位语义对齐。首个依赖该阈值的下游研究：
   `research/2026-09-14_phase-migration-th9/`（相位迁移 v4，日±5/周月±9）。
   原备选意见保留：若下游只是相对比较强弱/做排序，阈值影响有限，但要有
   防抖（相位判定在滚动口径下建议配合趋势值 MA5 或连续 N 日确认——滚动
   序列日频采样，单日穿越比 vendor 周期K 频繁得多）。
...
4. **对下游研究的口径提示**：引用滚动周/月分布数字时，注意 N 是日频
   ...
   阈值选择（§6.1）已定：2026-09-14 起日 ±5 / 滚动周月 ±9，落地首跑见
   `research/2026-09-14_phase-migration-th9/`。
...
| `data/stats.json` | 六组描述性统计 + ±5 分位 + 覆盖区间；滚动周/月四组另含 ±9 口径（2026-09-14 采纳阈值） | ✓ |
