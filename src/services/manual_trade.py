...
def compute_manual_trade(
    ...
    # 退化腿闸门：极短/近零方差的持有窗口会给出 1e2~1e14 级"夏普"，旧实现在这里
    # round() 后直接进 HTTP 响应（前端不渲染，但 API/MCP 消费者会拿到）。
    # 注：本函数**当前**未找到生产可达的退化构造——R12A 复扫 135 个买入窗口
    # 最大 |sharpe| = 11.2（此前报告里 57.4 的样例不可复现）；闸门是防御性的，
    # 与单跑/批量面共用同一幅值口径。
    # 只闸 `_RATIO_METRIC_KEYS`（sharpe/sortino）：calmar 低回撤/短窗口可合法 > 50。
    # `is_degenerate_summary` 已按同一键位做幅值判定（含 NAV 相对方差与现算 Sharpe
    # 两条腿），故此处**不再重复**一遍幅值兜底（R12A 复核：那条兜底分支是死代码）。
