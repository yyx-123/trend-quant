...
class TestKlineMini:
    ...
    def test_returns_last_forty_with_aligned_ma5(self) -> None:
        bars = self._bars(44)
        payload = kline_mini(bars)
        assert len(payload["kline"]) == 40
        assert len(payload["kline_ma5"]) == 40
        # 窗口从第 5 根（index 4）开始：close = 104 … 143
        assert payload["kline"][0]["c"] == 104.0
        assert payload["kline"][-1]["c"] == 143.0
        ...
        assert payload["kline_ma5"][-1] == pytest.approx((139 + 140 + 141 + 142 + 143) / 5)
